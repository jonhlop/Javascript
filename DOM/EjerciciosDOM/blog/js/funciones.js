...
function pintarNoticias(pTitulo, pNoticia) {
  ...
  let h3 = document.createElement("h3"); // crea el <h3> </h3>
  let tituloNoticia = document.createTextNode(pTitulo); // crea el contenido de dentro de <h3></h3>
  h3.appendChild(tituloNoticia); // mete el contenido en <h3></h3>
...
function borrarNoticia(pId) {
  ...
  var posicionBuscada = noticiasArray.findIndex(
    (nombre) => nombre.noticia == noticiaBuscar
  );
